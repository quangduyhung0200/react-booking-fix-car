...
import { updateHandbook } from '../../services/staffService';
...
class UpdateHandBook extends Component {
    ...
    handleSveConTen = async () => {
        ...
        let { avata, selectedOption } = this.state


        if (!this.state.contenHTML || !this.state.contenMarkdown || !this.state.title
            ...
            let garaId = this.state.selectGara.value
            if (garaId === undefined) {
                garaId = -1
            }
            let data = {
                staffId: this.context.user.account.id,
                contentHTML: this.state.contenHTML,
                    ...
                    <div className='register-body row'>
                        <div className='conten-left col-6'>
                            ...
                            <textarea className='form-control' rows={5}
                                onChange={(event) => this.handlOnChandText(event, 'title')}
...
UpdateHandBook.contextType = UserContext
