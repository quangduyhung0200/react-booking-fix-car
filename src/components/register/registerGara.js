    ...
    handleSveConTen = async () => {
        ...
        if (!this.state.contenHTML || !this.state.contenMarkdown || !this.state.description || !this.state.selectProvint || !nameGara || !addressGara || !phone
            ...
            if (this.isNumeric(phone) === false) {
                toast.error('Số điện thoại bạn nhập không hợp lệ')
                return
            }
            if (avata === '') {
            ...
            let data = await registerGara(nameGara, contenMarkdown, contenHTML, addressGara, provindId, avata, phone, description, emailUser, id)
            if (data.EC === 0) {
        ...
        return (
            <>
                ...
                <div className='register-container container'>
                    <div className='register-title text-centre'><h3>Đăng ký thông tin gara</h3></div>
                    <hr></hr>
                    <div className='register-body row'>

                        <label>Thông tin giới thiệu gara</label>
